...
export class SalaFormComponent {
  ...
  ngOnInit() {
    ...
    if (id) {
      ...
      this.salasService.getSalaById(+id).subscribe({
        next: (data: Sala) => (this.sala = data),
        error: (err: any) => console.error('Error al cargar la sala:', err)
      });
    }
  }
  ...
  guardar() {
    if (this.modoEdicion) {
      this.salasService.updateSala(this.sala).subscribe({
        next: () => {
          alert('Sala actualizada correctamente');
          this.router.navigate(['/admin/salas']);
        },
        error: (err: any) => console.error('Error al actualizar sala:', err)
      });
    } else {
      this.salasService.addSala(this.sala).subscribe({
        ...
        error: (err: any) => console.error('Error al crear sala:', err)
      });
    }
  }
